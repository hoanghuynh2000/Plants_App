...
class FirListOder {
  ...
  Future<void> addListOrder(
    ...
    await oderId.get().then((value) => value.docs.forEach((element) {
          itemList.add(mdOrder.fromJson(element.data()));
        }));
    ...
    mdorder.state = state;
    mdorder.date = date;
    mdorder.dateDelivery = 'Chưa xác nhận';
  ...
  Future updateAddress(
      String idOrder, String name, String phone, String address) async {
    String? id;
    FirebaseAuth auth = FirebaseAuth.instance;
    String uid = auth.currentUser!.uid.toString();
    final shoppingCartId = FirebaseFirestore.instance
    ...
    FirebaseFirestore.instance
        .collection('Orders')
        .doc(id)
        .update({'nameCus': name, 'phoneCus': phone, 'addressCus': address});
  }
  ...
  Future removeProduct(String idOder) async {
    ...
    final shoppingCartId = FirebaseFirestore.instance
        .collection('Orders')
        .where('idOrder', isEqualTo: idOder);
    await shoppingCartId.get().then((value) => value.docs.forEach((element) {
          id = element.id;
        }));
    // print(shoppingCartId.id);
    final deshoppingCartId =
        FirebaseFirestore.instance.collection('Oders').doc(id).delete();
  }
}
