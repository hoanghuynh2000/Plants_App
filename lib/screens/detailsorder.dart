...
class _DetailsOrderState extends State<DetailsOrder> {
  ...
  Widget build(BuildContext context) {
    ...
    List<mdOrder>list=fakeListOrder.where((element) => element.idOrder==widget.mdDetail).toList();
    List<DetailProduct>listPro=FakeProduct.toList();
    return Scaffold(
      backgroundColor: Colors.grey.shade200,
      appBar: AppBar(title: Text('Chi tiết đơn hàng'),backgroundColor:Colors.teal.shade900),
        ...
        Container(
              ...
              children: [
                ...
                Expanded(child:  Column(
                  ...
                  children: [
                    ...
                    Text('${NumberFormat('###,###').format(list[0].totalPrice)}',style: TextStyle(fontSize: 16)),
                    Text('${NumberFormat('###,###').format(list[0].costShip)}',style: TextStyle(fontSize: 16)),
                    Text('-${NumberFormat('###,###').format(list[0].promotion)}',style: TextStyle(color: Colors.red.shade800, fontSize: 16)),
                    SizedBox(height: 10,),
                    Text('${NumberFormat('###,###').format(list[0].totalPrice)}',style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold)),
                  ],
                ))
         ...
         Container(
          ...
          decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(20)),color: Colors.white),
           ...
           Column(
             crossAxisAlignment: CrossAxisAlignment.start,
             children: [
               Text('Mã đơn hàng: ${list[0].idOrder}',
               style: TextStyle(
         ...
         Row(
          mainAxisAlignment: MainAxisAlignment.center,
           ...
           onPressed: (){},child: Text('Hủy Đơn Hàng',style: TextStyle(color: Colors.white,fontSize: 15,fontFamily: 'Lemonada'),),),
             )
